...
func NewMCPServer(provider *provider.ApiProvider, logger *zap.Logger, enabledTools []string) *MCPServer {
	...
	if shouldAddTool(ToolConversationsAddMessage, enabledTools, "SLACK_MCP_ADD_MESSAGE_TOOL") {
		s.AddTool(mcp.NewTool(ToolConversationsAddMessage,
			mcp.WithDescription("Add a message to a public channel, private channel, or direct message (DM, or IM) conversation by channel_id and thread_ts. With the default content_type text/markdown the text is converted to native Slack rich_text before posting. Lists: `-`, `*`, `1.` and `1)` markers AND the bullet characters `•`, `◦`, `‣`, `⁃`, `·` all produce a real Slack list when they start a line and are followed by a space — no need to rewrite bullets into dashes. Mentions: `<@U…>`, `<#C…>` and `<!subteam^S…>` become live mentions, and the referenced people ARE notified when the message posts, so write only references you intend to ping. `@here`, `@channel` and `@everyone` are always written as inert plain text and can never raise a workspace-wide alert through this tool. Every mention is verified before anything is posted: if a reference names nothing in this workspace, or the workspace cannot be consulted to check it, the call fails naming that exact reference (and distinguishing \"does not exist\" from \"could not verify\") and no message is sent — correct the reference and retry. content_type text/plain and caller-supplied blocks are verbatim paths, exempt from both the markdown conversion and the reference check."),
	...
	if !provider.IsOAuth() && shouldAddTool(ToolConversationsDraftMessage, enabledTools, "SLACK_MCP_DRAFT_MESSAGE_TOOL") {
		s.AddTool(mcp.NewTool(ToolConversationsDraftMessage,
			mcp.WithDescription("Create or update a native Slack draft message in a channel, DM, or thread. Non-destructive by default: if a draft already exists at the destination, nothing is written and its content is returned (action: existing_draft_found). Then compare the returned blocks_json against the blocks_json from your own last successful call for this destination — byte-identical means it is your own untouched draft, so re-call with overwrite: true without asking the user; different, or you never wrote there, means show the user the readable text and get their explicit consent before re-calling with overwrite: true. blocks_json is the authoritative content; text is display-only — never restore or compare from text. To restore displaced content exactly, re-call with content_type: application/json and the saved blocks_json as text; replaced results include the displaced content for exactly that purpose. Scheduled drafts are never replaced or deleted under any circumstances. With the default content_type text/markdown the text is converted to native Slack rich_text. Lists: `-`, `*`, `1.` and `1)` markers AND the bullet characters `•`, `◦`, `‣`, `⁃`, `·` all produce a real Slack list when they start a line and are followed by a space — no need to rewrite bullets into dashes. Mentions: `<@U…>`, `<#C…>` and `<!subteam^S…>` become live mentions, and the referenced people ARE notified once the user sends the draft, so write only references you intend to ping. `@here`, `@channel` and `@everyone` are always written as inert plain text and can never raise a workspace-wide alert through this tool. Every mention is verified before any draft API call is made: if a reference names nothing in this workspace, or the workspace cannot be consulted to check it, the call fails naming that exact reference (and distinguishing \"does not exist\" from \"could not verify\") — nothing is written, and a draft already at the destination is not read, not listed, and not modified. content_type text/plain and application/json are verbatim paths, exempt from both the markdown conversion and the reference check. Requires a session token (xoxc/xoxd); drafts are saved to the user's Drafts and NOT sent automatically."),
	...
	conversationsSearchTool := mcp.NewTool(ToolConversationsSearchMessages,
		...
		mcp.WithString("filter_has_reaction",
			mcp.Description("Filter messages that have a specific emoji reaction from anyone. Accepts an emoji name with or without colons, e.g. 'eyes' or ':eyes:'. If not provided, messages are not filtered by reactions."),
		),
		mcp.WithString("filter_my_reaction",
			mcp.Description("Filter messages that have a specific emoji reaction added by you (the authenticated user). Accepts an emoji name with or without colons, e.g. 'pushpin' or ':pushpin:'. Useful to find messages you bookmarked by reacting. If not provided, messages are not filtered by your reactions."),
		),
		mcp.WithString("cursor",
			mcp.DefaultString(""),
			mcp.Description("Cursor for pagination. Use the value of the last row and column in the response as next_cursor field returned from the previous request."),
