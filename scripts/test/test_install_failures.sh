...
case_probe_failure_rc5_keeps_existing_binary() {
  t_sandbox
  stage_release pv-v1.2.3 bad-banner # checksum valid, --version banner wrong
  map_latest pv-v1.2.3
  printf '#!/bin/sh\necho existing\n' >"$PREFIX/slack-mcp-server"
  chmod +x "$PREFIX/slack-mcp-server"
  local pre
  pre="$(sha256_of "$PREFIX/slack-mcp-server")"
  local rc=0
  run_install "$WORK/out" --no-updater || rc=$?
  assert_rc 5 "$rc"
  assert_contains "$WORK/out" "existing installation unchanged"
  assert_exec "$PREFIX/slack-mcp-server"
  if [ "$(sha256_of "$PREFIX/slack-mcp-server")" != "$pre" ]; then
    echo "ASSERT: existing binary was modified by a failed install"
    return 1
  fi
}
...
t_case probe_failure_rc5_keeps_existing_binary case_probe_failure_rc5_keeps_existing_binary
t_done
