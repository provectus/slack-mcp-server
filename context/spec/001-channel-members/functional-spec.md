...
- **As** the assistant, **I want to** list the members of a specific channel, **so that** I can tell the user who belongs to it and reason about the conversation.
  - **Acceptance Criteria:**
    - [x] Given a channel the assistant can see, when the assistant requests that channel's members, then it receives a list where each entry shows the member's user ID, display name, and real name.
    - [x] When the requested channel has many members, then the response contains the complete roster (every member is returned, not a partial page).
    - [x] Given an empty or members-hidden channel, when the assistant requests its members, then it receives an empty list rather than an error.

- **The user identifies the channel by either its ID or its name.**
  - **Acceptance Criteria:**
    - [x] When the assistant supplies a channel ID (e.g. `C0123ABCD`), then the members of that channel are returned.
    - [x] When the assistant supplies a channel name (e.g. `#general` or `general`), then the same members are returned as if the ID had been supplied.
    - [x] Given a channel reference that matches no known channel, when the assistant requests its members, then it receives a clear message saying the channel was not found, and no partial or misleading list.

- **The member names come from the same source as the rest of the product, so they are consistent.**
  - **Acceptance Criteria:**
    - [x] When a member is shown, then their display name and real name match what the assistant already reports elsewhere for that same user.
    - [x] Given a member whose profile details are not yet known locally, when the roster is returned, then that member still appears (by ID), with whatever name information is available, rather than being dropped.

- **The assistant can narrow the list to exclude bots and deactivated accounts.**
  - **Acceptance Criteria:**
    - [x] By default, when the assistant requests a channel's members, then everyone the channel reports is included — real people, bots, and deactivated accounts.
    - [x] When the assistant asks to exclude bots, then bot users are omitted and all other members remain.
    - [x] When the assistant asks to exclude deactivated accounts, then deactivated users are omitted and all other members remain.
    - [x] When the assistant asks to exclude both, then only active human members remain.

- **The roster is remembered so repeat requests are fast.**
  - **Acceptance Criteria:**
    - [x] Given a channel whose members were requested recently, when the assistant requests them again, then the answer is served from the remembered roster without a fresh Slack lookup.
    - [x] The remembered roster survives a restart of the server (a repeat request after restart is still fast and does not require re-fetching from Slack, until it ages out).
    - [x] When the remembered roster has aged past its freshness window, then the next request transparently refreshes it and returns the up-to-date roster.
    - [x] When the assistant explicitly asks to refresh a channel's members, then the roster is re-fetched from Slack and the returned list reflects the current membership.
    - [x] Remembered rosters are kept separately per workspace, so members from one workspace are never shown for another.

- **While the roster is first being gathered, the assistant is told to wait rather than shown a wrong answer.**
  - **Acceptance Criteria:**
    - [x] Given a channel whose roster has never been gathered and a background gather is in progress, when the assistant requests its members, then it receives a short "still preparing, please retry shortly" message rather than an empty or partial list.
